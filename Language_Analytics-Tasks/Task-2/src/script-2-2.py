...
def main():
    tracker = EmissionsTracker(output_file = "emissions_assignment_2.csv", output_dir = "out/emissions/", project_name = "neural_classification")
    tracker.start()
    ...
    tracker.start_task("split_neural")
    splits = functions.get_splits(data)
    train_X = splits[0]
    test_X = splits[1]
    train_y = splits[2]
    test_y = splits[3]
    tracker.stop_task("split_neural")
    ...
    tracker.start_task("classification_neural")
    functions.neural_classification(train_X_features, train_y, test_X_features, test_y)
    tracker.stop_task("classification_neural")
